...
const selling_projects = () => {
  ...
  if (location.trim() !== "") {
    filterProduct = filterProduct.filter((p) =>
      p.location.toLowerCase().includes(location.toLowerCase()) || p.bhk.includes(location)
    );
  }
  ...
  if(categoryType){
    filterProduct=filterProduct.filter((p)=>
       p.property_condition===categoryType
    );
  }
  if (sortLowHigh) {
      ...
      <div className="flex flex-col gap-2 border w-full max-w-lg mx-auto px-5 sm:px-10">
        ...
        <div className='p-5'>
          ...
          <input 
            ...
            placeholder='search location/bhk'
          />
        </div>
...
<div className='p-5'>
  ...
  <label className="flex items-center gap-2">
    <input 
      ...
      onChange={() => setCategoryType(categoryType === "new" ? "" : "new")}
    />
  ...
  <label className="flex items-center gap-2">
    <input 
      type="checkbox" 
      checked={categoryType === "old"} 
      onChange={() => setCategoryType(categoryType === "old" ? "" : "old")}
    />
...
export default selling_projects;
